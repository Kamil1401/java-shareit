...
class BookingControllerTest {
    ...
    void addBooking() throws Exception {
        ...
        ItemShortDto itemShortDto = new ItemShortDto(1L, "Дрель", 2L);
        UserShortDto userShortDto = new UserShortDto(17L, "Кеша");

        BookingDto bookingDto = BookingDto.builder()
                .id(5L)
                .start(LocalDateTime.of(2026, 1, 1, 10, 0))
                .end(LocalDateTime.of(2026, 1, 1, 10, 30))
                .status(BookingStatus.WAITING)
                .item(itemShortDto)
                .booker(userShortDto)
                .build();

        when(bookingService.create(eq(17L), any()))
                .thenReturn(bookingDto);

        mvc.perform(post("/bookings")
                        .header("X-Sharer-User-Id", 17L)
    ...
    void confirmBooking() throws Exception {
        ...
        when(bookingService.confirmBooking(eq(14L), eq(4L), eq(true)))
                .thenReturn(bookingDto);
